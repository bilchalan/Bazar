...
exports.updateProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  //if 
  if (!req.body.checkImages) {
    return next(new errorHandler("Missing check images parameter!", 400));
  }

  if(req.body.checkImages!=="NotChanged")
  {
        //FOR NEW IMAGES--------------
        let images = [];
        if (typeof req.body.images === "string") {
        ...
        for(let i=0;i<product.images.length;i++)
        {
          imagesLinks.push({public_id:product.images[i].public_id,url:product.images[i].url});
        }
        req.body.images = imagesLinks;
  }
...
exports.createProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  const review = {
    user: req.user._id,
    name: req.user.name,
    avatar_url:req.user.avatar.url,
